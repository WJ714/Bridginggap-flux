...
library(Kendall)
# working directory
data_dir = ("C:/Users/yl763/Documents/GitHub/FCC_workshop_flux_test/FLUXNET_US-Syv/") # Yujie's local path
# data_dir = "~/"
```
...
```{r}
# setwd(data_dir); df.HH = read.csv(paste0("AMF_US-Syv_FLUXNET_SUBSET_HH_2001-2023_4-6.csv")) 
# df.HH$TIMESTAMP_END <- ymd_hm(as.character(df.HH$TIMESTAMP_END))
# df.HH <- df.HH %>%
#   mutate(year = year(TIMESTAMP_END), month = month(TIMESTAMP_END))
# df.HH <- df.HH[df.HH$year >= 2012 & df.HH$year <= 2023, ]
# setwd(data_dir); write.csv(df.HH, "AMF_US-Syv_FLUXNET_SUBSET_HH_2012-2023_4-6.csv")
