...
ggplot(df.YY, aes(x = year)) +
  # Uncertainty ribbon
  geom_ribbon(aes(ymin = NEE_lower, ymax = NEE_upper), fill = "blue", alpha = 0.4) +
...
data_with_labels %>%
  ...
  labs(
    title = "NEE by 30-mins and quality flags",
    x = "DOY",
    y = expression(FCO2~(g~C~m^{-2}~d^{-1}))
  ) + my_theme
```

# Optional: Ecosystem water budget
ET = Evapotranspiration = the sum of evaporation from land + transpiration from plants.
