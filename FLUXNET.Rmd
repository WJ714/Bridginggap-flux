...
# How crediable the annual sums are?
plot(df.YY$year, df.YY$NEE_VUT_REF_QC, las = 1, main = "qualify flag for YY data")
```
...
# Working with daily data (to be updated)
```{r}
...
print(fit_light_response_NEE(data_filtered))
...
ggplot(results_df, aes(x = as.numeric(Year), y = A2000)) +
  ...
  theme_minimal() +
  ylim(0,50) # I will double-check with Andrew if this makes sense
```
Ideas for follow-up analysis:
