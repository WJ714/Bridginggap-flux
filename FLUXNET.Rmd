...
monthly_data <- monthly_data %>%
  mutate(y_label_pos = max(GPP, RECO, NEE, na.rm = TRUE) * 1.2)

ylab = expression(FCO[2] ~ "(" * mu * "mol" ~ m^{-2} ~ month^{-1} * ")")
...
## Task 3: qualify flags of half-hourly data
```{r}
...
## Task 4: monthly meterological variables
```{r}
# Tair -----------------------------------------------------
monthly_data <- monthly_data %>%
