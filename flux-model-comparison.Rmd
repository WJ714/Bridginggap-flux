...
# OBJECTIVES
  -visualize the ELM model output vs FLUXNET tower data relationship using time-series and x-y scatter plots
  -evaluate model-data agreement using a simplified approach 
...
{
  file_url <- "https://drive.google.com/file/d/1kGu2JTe5afS_6ly9p3xgVFtTVtpmc__4/view?usp=drive_link"
...
# DOWNLOAD AND READ DEFAULT ELM OUTPUT
{
  ...
  nc_file_url <- "https://drive.google.com/file/d/1eCR99plnQQpqqQ0Y1h6YZlVA-8fBfgJ0/view?usp=drive_link"
...
{
  # Google Drive URL for the .nc file
  nc_adj_file_url <- "https://drive.google.com/file/d/1Xxv2Z8MF3pbo6e0mm_s5SkyTTpTGX1xE/view?usp=drive_link"
  nc_adj_file_id <- as_id(nc_adj_file_url)
...
# combine flux data and model output
df.model = df.model[df.model$year >= 2001, ] # please change to the start year of flux measurement for you study site
df.model_adj = df.model_adj[df.model_adj$year >= 2001, ] # please change to the start year of flux measurement for you study site
df.model_adj <- df.model_adj %>%
...
## Edit this chunk as needed
## The example here is currently set for comparing air temperature
## Use the crosswalk table to plot other micrometeorological variables (i.e. atmospheric pressure, air temperature, precipitation)
```{r}
# ===== USER INPUT SECTION ====================================================
...
selected_years <- list(2001:2015) #change years to plot
# =============================================================================
...
## Energy exchange variables comparison
## Edit this chunk as needed
## The example here is currently set for comparing Latent heat flux
## Use the crosswalk table to plot other energy exchange variables 
```{r}
# ===== USER INPUT SECTION ====================================================
...
## Flux variables comparison
## Edit this chunk as needed
## The example here is currently set for comparing NEE
## Use the crosswalk table to plot other flux variables 
```{r}
# ===== USER INPUT SECTION ====================================================
...
time_series_label <- expression(NEE~(W/m2)) # change label name and units as needed
...
ggplot(df.cumulative, aes(x = TIMESTAMP)) +
  ...
  labs(
    ...
    x = "Date",
    y = expression(Cumulative~GPP~(gC/m^2)),
    color = "Source"
  ) +
...
for (i in seq(1, length(all_years), by = 4)) {
  ...
  p_cumsum <- ggplot(df_subset, aes(x = doy.x, y = cumulative_value, color = source)) +
    geom_line(size = 1) +
    facet_wrap(~ year.x, ncol = 2) +
    ylab(expression(Cumulative~GPP~(gC/m^2))) +
    scale_color_manual(
      values = setNames(c("red", "blue"), c(flux_col, model_col)),
      labels = c("Flux", "Model")
    ) +
...
NOTE: this is a simplified approach that evaluates model-data agreement assuming model and obs data are from a single point, per year, and over the entire period that data is available for both. This means that there aren't spatial components to the evaluation. 
...
selected_years <- list(2002:2006, 2012:2015) #change years to plot; it can handle several batches (ex: list(2001:2005, 2010:20015))
# =============================================================================
...
## OPTIONAL: evaluate flux vs model per month (across years); are there months in the year when the model and flux output agree better?
```{r}
# Variables to evaluate
variable_list <- c("GPP", "NEE", "Reco")
