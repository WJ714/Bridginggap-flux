...
# # setwd(project_dir); fwrite(df.subset, "US-Syv_NEE.csv")
# YUJIE: let me double-check with Andrew ...
...
for (variable_run in variable_list) {
  ...
  # Overall score
  S <- (domain_S_bias + 2 * S_rmse + S_dist) / 4
...
scores <- c("correlation score", "bias score", "RMSE score", "overall score") #only want to make heatmap of scores, other stuff will be table
...
metric_df_sub$metric <- factor(metric_df_sub$metric, levels = c("bias score", "correlation score", "RMSE score", "overall score"))


p <- ggplot(metric_df_sub, aes(x = metric, y = site, fill = value)) +
...
#ggsave(filename = plot_save, plot = p, dpi = 500)
```
