...
```
# Yujie: For the working directory, I google a bit, it might be possible after we make the repo public??? I have not tested it yet.
url <- "https://raw.githubusercontent.com/yourusername/yourrepo/main/data/yourfile.csv"
df <- read.csv(url)
...
# check data coverage for meterology
...
- How many sites are included in the BASE dataset?
- For US-Bar, what do you observe for data coverage for meteorology and flux data respectively?
...
```
# 05: u* filtering
- u*: Friction velocity (m/s) is a measure of turbulent mixing in the atmosphere;
- u* is defined as a function of the fluctuations of horizontal and vertical wind speeds (Yujie: I am not sure if this is accurate definition)
...
- To filter out period with low turbulence, e.g., at night, when winds are calm, the atmosphere often becomes stable with low turbulence;
...
- u* threshold is the minimum u* above which respiration reachers a plateau; (check out the figure under /plots)
...
Question to audience: can I use more than 3 drivers to gapfill NEE? 
```{r}
...
- nighttime partitioning: doi
- Reco is modelled and GPP is computed as Reco - NEE; 
add text: diffeernt partitiong method: 
```{r}
...
- add LE and H: The differences are that no u filtering is not generally applied, and the calculation of annual sums would be different.
- Using different drivers: I am always curious about how to do that... It is a change to have Thomas involved if he would like to help ... Or I guess it might be possible to adapt the codes for step by step for MDS here: https://cran.r-project.org/web/packages/REddyProc/vignettes/gapFilling.html
- more about quality flags?


# 11: Next step
- organse this script better

After meeting with other organizers: 
...
- LE and H: LSM model output? format of LSM model ... 
- 
