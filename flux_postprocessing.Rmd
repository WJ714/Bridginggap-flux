...
# # Package for creating formatted reports (tables, etc.)
# library(pander)  # Generate pretty tables for reports or console outputs.
...
```
Q: What do you think about the flux data coverage at US-Bar? (we can ask some question to get audience attention)
...
EProc$sPlotFingerprintY("GPP_50%_f", Year = 2012)
```
...
```
# 09: compute annual and monthly sums
...
agg.annual = combined.df %>%
    ...
    summarize(annual_FC = sum(NEE_50._f * molarMass * 30 * 60 / 1e6),
              annual_GPP = sum(`GPP_50%_f` * molarMass * 30 * 60 / 1e6), # there are NAs in GPP???
...
```
# 10: optional: 
Here are some thoughts from Yujie:
- add LE and H: The differences are that no u filtering is applied, and the calculation of annual sums differs compared to FCO₂.
- Using different drivers: I am always curious about how to do that.. It is a change to ask Thomas if he would like to help ...
```{r}
...
# 11: next step
- format output similar to FLUXNET?
- download and organise FLUXNET data from our study sites (+ some figures about the annual variation ...?)
